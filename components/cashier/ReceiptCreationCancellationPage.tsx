...
const ReceiptCreationCancellationPage: React.FC = () => {
    ...
    const [selectedType, setSelectedType] = useState<'INVESTMENT' | null>(null);
    ...
    useEffect(() => {
        if (showCreateModal && selectedType === 'INVESTMENT') {
    ...
    const handleIssueReceipt = async () => {
        ...
        try {
            ...
            toast.success("Investment Receipt Issued Successfully!");
                            ...
                            {!selectedType ? (
                                <div className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div className="p-6 bg-muted-bg/30 border border-border-default/50 rounded-3xl opacity-50 cursor-not-allowed space-y-3">
                                            <div className="w-12 h-12 rounded-2xl bg-gray-500/10 flex items-center justify-center text-gray-500">
                                                <FileText size={24} />
                                            </div>
                                            <div>
                                                <h4 className="font-black text-text-primary">General Inflow</h4>
                                                <p className="text-xs text-text-muted font-medium italic">Logic pending merge...</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ) : selectedType === 'INVESTMENT' && !selectedInv ? (
                                <div className="space-y-6">
                                    <div className="flex items-center justify-between">
                                        <p className="text-sm font-bold text-text-muted uppercase tracking-widest">Select Authorized Asset</p>
                                    ...
                                    <div className="space-y-3 max-h-[300px] overflow-y-auto">
                                        {loadingInvestments ? (
                                            ...
                                            <div className="py-10 text-center border-2 border-dashed border-border-default rounded-3xl">
                                                <p className="text-text-muted text-sm font-bold">No authorized investments awaiting payment.</p>
                                            </div>
                                        ) : (
                                            filteredInvestments.map(inv => (
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        setSelectedInv(inv);
                                                        setReceiptId(`RCP-INV-${Date.now().toString().slice(-6)}`);
                                                    }}
                                                ...
                                                >
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div>
                                                            <h5 className="font-black text-text-primary text-sm">{inv.customer?.full_name}</h5>
                                                            <p className="text-[10px] text-text-muted font-bold tracking-tight">{inv.product?.name} • {inv.transaction_id}</p>
                                                        </div>
                                                    </div>
                                                    <div className="text-right">
                                                        <p className="text-sm font-black text-text-primary">LKR {Number(inv.amount).toLocaleString()}</p>
                                ...
                                <div className="space-y-8 animate-in slide-in-from-bottom-2 duration-300">
                                    ...
                                    <div className="bg-emerald-500/5 p-6 rounded-3xl border border-emerald-500/10 space-y-4">
                                        <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-text-muted">
                                            ...
                                            <span className="text-text-primary">{selectedInv.customer?.full_name}</span>
                                        </div>
                                        <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-text-muted">
                                            <span>Financial Asset</span>
                                            <span className="text-text-primary">{selectedInv.product?.name}</span>
                                        </div>
